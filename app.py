...
st.markdown("""
    ...
    .stDownloadButton button { background-color: #9B8ADE !important; color: white !important; border-radius: 8px !important; width: 100%; height: 50px; font-weight: bold; }
    </style>
...
if arquivo:
    with st.spinner('SOLUX está processando... 🕵️‍♂️✨'):
        try:
            ...
            if banco:
                ...
                with pd.ExcelWriter(out, engine='xlsxwriter') as writer:
                    ...
                    for cod, df_emp in banco.items():
                        ...
                        ws.hide_gridlines(2)
                        ws.set_column('B:C', 15); ws.set_column('D:D', 45); ws.set_column('E:F', 18); ws.set_column('I:M', 18)
                        ...
                        for ri, r in enumerate(df_emp.values):
                            ...
                            ws.write(6+ri, 3, r[2], fmt_s); ws.write_number(6+ri, 4, r[3], fmt_m); ws.write_number(6+ri, 5, r[4], fmt_m)
                            row_f_razao = 6+ri
                        
                        # PULANDO LINHA NOS TOTAIS DO RAZÃO
                        ws.write(row_f_razao + 2, 3, "TOTAL RAZÃO:", f_cab)
                        ws.write_number(row_f_razao + 2, 4, df_emp["Deb"].sum(), f_m)
                        ws.write_number(row_f_razao + 2, 5, df_emp["Cred"].sum(), f_m)
                        
                        ws.write(row_f_razao + 3, 4, "SALDO LÍQUIDO:", f_cab)
                        saldo_r = df_emp["Deb"].sum() + df_emp["Cred"].sum()
                        ws.write_number(row_f_razao + 3, 5, saldo_r, f_vde if abs(saldo_r) < 0.01 else f_vrm)
                        ...
                        for ri, r in enumerate(res.values):
                            ...
                            ws.write(6+ri, 12, status, wb.add_format({'align': 'center', 'bold': 1, 'font_color': 'green' if status=="OK" else 'red', 'border': 1}))
                            row_f_res = 6+ri
                        
                        # PULANDO LINHA NO SALDO DA CONCILIAÇÃO
                        ws.write(row_f_res + 2, 11, "SALDO FINAL:", f_cab)
                        saldo_c = res["Dif"].sum()
                        ws.write_number(row_f_res + 2, 12, saldo_c, f_vde if abs(saldo_c) < 0.01 else f_vrm)

                st.success("✅ Relatório gerado com os espaços solicitados!")
